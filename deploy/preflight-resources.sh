...
readonly EXPECTED_K_GRAPH_MCP_HOST_PORT=9013
...
readonly DEFAULT_K_GRAPH_MCP_HOST_PORT="${EXPECTED_K_GRAPH_MCP_HOST_PORT}"
...
apply_default_port_values() {
  ...
  K_GRAPH_MCP_HOST_PORT="${K_GRAPH_MCP_HOST_PORT:-${DEFAULT_K_GRAPH_MCP_HOST_PORT}}"
}
...
check_reserved_port_values() {
  ...
  [[ "${K_GRAPH_MCP_HOST_PORT}" == "${EXPECTED_K_GRAPH_MCP_HOST_PORT}" ]] || fail "K_GRAPH_MCP_HOST_PORT must stay ${EXPECTED_K_GRAPH_MCP_HOST_PORT}"
...
check_artifacts() {
  ...
  for path in \
    ...
    "${ROOT_DIR}/mcp/servers/k-graph-mcp/Dockerfile" \
