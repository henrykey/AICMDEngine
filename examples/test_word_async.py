...
async def test_word_reading():
    ...
    try:
        # 添加标题
        await content_tools.add_heading(
            filename=test_file,
            text="第一章：测试标题",
            level=1
        )
    ...
    print("-" * 50)
    try:
        result = await document_tools.get_document_text(test_file)
        print(result)
    except Exception as e:
        print(f"❌ 读取失败: {e}")
    ...
    print("-" * 50)
    try:
        result = await document_tools.get_document_info(test_file)
        print(result)
    except Exception as e:
        print(f"❌ 读取失败: {e}")
    ...
    print("-" * 50)
    try:
        result = await document_tools.get_document_outline(test_file)
        print(result)
    except Exception as e:
        print(f"❌ 读取失败: {e}")
